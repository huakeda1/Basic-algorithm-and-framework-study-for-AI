...
import json
import os
from util.allennlp_file_utils import cached_path as allennlp_cached_path


def cached_path(file_path, cached_dir=None):
    if not cached_dir:
        #cached_dir = str(Path(Path.home() / '.convlab2') / "cache")
        cached_dir=os.path.join(os.path.dirname(os.path.dirname(os.path.realpath('__file__'))),'cache')
